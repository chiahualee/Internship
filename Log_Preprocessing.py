...
def utilization_graph_data(fileName,df):
    ...
    if "gpu" in fileName:
        ...
        temp.to_csv("daily_logs/"+var1+"_"var2+"gpu_util.csv")
        
    elif "rm" in fileName:
        all_data_df2["partition"]=all_data_df2["partition"].fillna("NA")
        temp=all_data_df2.groupby(['time',"core","partition","state"])['Node'].count().reset_index()
        temp.to_csv("daily_logs/"+var1+"_"var2+"rm_util.csv")
        ...
        temp.to_csv("daily_logs/"+var1+"_"var2+"lm_util.csv")
...
def backlog_graph_data(fileName,df):    
    ...
    if "gpu" in fileName:
        result_small=result.iloc[:,[0,4,9,12,16]]
        result_small.columns=["time","nodeType","partition","reqTime","user"]
    elif "rm" in fileName:
        result_small=result.iloc[:,[0,9,12,16]]
        result_small.columns=["time","partition","reqTime","user"]
    else:
        result_small=result.iloc[:,[0,3,9,12,16]]
    ...
    if "gpu" in fileName:
        ...
        result_small.to_csv("daily_logs/"+var1+"_"var2+"backlog_gpu.csv")
    elif "rm" in fileName:
        
        result_small.to_csv("daily_logs/"+var1+"_"var2+"backlog_rm.csv")

    else:
        result_small.to_csv("daily_logs/"+var1+"_"var2+"backlog_lm.csv") 
...
def Interact_graph_data(fileName,df): 
    ...
    if "gpu" in fileName:
        df_interact.to_csv("daily_logs/"+var1+"_"var2+"interact_gpu.csv")
    elif "rm" in fileName:
        df_interact.to_csv("daily_logs/"+var1+"_"var2+"interact_rm.csv")        
    else:
        df_interact.to_csv("daily_logs/"+var1+"_"var2+"interact_lm.csv") 
